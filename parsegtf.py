...
def makeExonIntronTree(gtf, pchrom):
    ...
    for line in gfh:
        ...
        genes.setdefault(transcript,dict()).setdefault('exon',list()).append((posstrt,posstop))
    ...
    for transcript in genes:
        ...
        for i,exon in enumerate(elist):
            ...
            node = intervalTree.Tree(mi=exon[0],ma=exon[1],name=transcript,name2=genes[transcript]['gene'],strand=genes[transcript]['strand'],misc=exord)
        ...
        for i,intron in enumerate(ilist):
            ...
            node = intervalTree.Tree(mi=intron[0],ma=intron[1],name=transcript,name2=genenm,strand=genes[transcript]['strand'],misc=inord)
            root = node.insertTree(root)
            treeroots[chrom] = root
    return treeroots
        ...
        for i,exon in enumerate(elist):
            if genes[transcript]['strand'] == '+':
                ...
                if i == 0:
                    ...
                    exord = 'middle'
            node = intervalTree.Tree(mi=exon[0],ma=exon[1],name=transcript,name2=genes[transcript]['gene'],strand=genes[transcript]['strand'],misc=[exord,(elist[0][0],elist[-1][1])])
            root = node.insertTree(root)
            treeroots[chrom] = root
        for i,intron in enumerate(ilist):
            if genes[transcript]['strand'] == '+':
                ...
                if i == 0:
                    ...
                    inord = 'middle intron'
            node = intervalTree.Tree(mi=intron[0],ma=intron[1],name=transcript,name2=genenm,strand=genes[transcript]['strand'],misc=[inord, (ilist[0][0],ilist[-1][1])])
            root = node.insertTree(root)
            treeroots[chrom] = root
    return treeroots
...
def makeGeneTreeFromExons(gtf, pchrom):
    ...
    for transcript in genes:
        ...
        if chrom in treeroots:
            ...
            root = None
        # for i,exon in enumerate(elist):
        node = intervalTree.Tree(mi=strt,ma=end,name=transcript,name2=genes[transcript]['gene'],strand=genes[transcript]['strand'])
        root = node.insertTree(root)
        treeroots[chrom] = root
    return treeroots
...
def getGeneLengthDistribution(gtf,limit):
    ...
    matplotlib.pyplot.hist(numpy.array(glen,dtype=float),range=limit,bins=100,color = 'lightskyblue')
