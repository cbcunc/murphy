...
# intevalTree.py is an adaptation of the interval tree algorithm based on red-black trees from
# Introduction to Algorithms by Cormen, Leiserson, Rivest and Stein (2001)
# 2nd Edition, The MIT Press

# This makes a nice self-balancing tree.  If items to be inserted are already sorted,
# the alrogithm may be more efficient if items are ranomly selected for insertion.

# I've modified the CLRS algorithm to report all overlapping nodes instead of
# only the first node.  This involved adding a min value instead of only a max value
# in order to speed up the search by checking if the subtree min,max overlaps with
# the search interval.  The search routine is also recursive.
#
# 2010-05-31 S. Stiegelmeyer Created
# 2014-08-21 S. stiegelmeyer Added closestNode method to find closest interval to 
#                            non-overlapping interval
# 2014-09-29 S. Stiegelmeyer Fixed bugs in closestNode method and removed rightChild method
...
def traverseInOrder(node):
    """ In Order traversal of tree.  Should see list from smallest to largest by x value.
        Input:
...
def searchTree(node,mi,ma):
    ...
    if overlap(mi,ma,node.x,node.y):
        ret.append(node)
    if node.right is not None and overlap(mi,ma,node.right.min,node.right.max):
...
def closestNode(node,mi,ma):
    ...
    while node is not None:
        if overlap(mi,ma,node.x,node.y):
            node=None
            prev=None
        elif node.left is not None and overlap(mi,ma,node.left.min,node.left.max):
            prev=node
            node=node.left
        elif node.right is not None and overlap(mi,ma,node.right.min,node.right.max):
            prev=node
            node=node.right
        else:
            prev=node
...
class Tree:
    ...
    def __init__(self,mi=0,ma=0,tss=0,tes=0,strand="-",name="",name2="",\
                 cdsS=0,cdsE=0,eTree=None,eType=0,misc=[]):
        self.x=mi
        self.y=ma
        self.max=ma
        self.min=mi
    ...
    #    return self.x==other.x and self.y==other.y and self.max==other.max

    def __repr__(self):
        return "({0.x!r}, {0.y!r}),({0.cdsStart!r},{0.cdsEnd!r}),{0.exontype!r}".format(self)

    def __str__(self):
        return "({0.x!r}, {0.y!r}, {0.max!r}) {0.color!r} {0.name!r} {0.strand!r} {0.exontype!r}".format(self)
    ...
    def insertTree(self,root):
        ...
        if nodesave is None:
            root=self
        elif self.x < nodesave.x or (self.x==nodesave.x and self.y >= nodesave.y):
            nodesave.left=self
        else:
            nodesave.right=self
        self.left=None
    ...
    def insertFixup(self,root):
        ...
        while z is not None and z.p is not None and z.p.color==RED:
            if z.p.p is not None and z.p==z.p.p.left:
                ...
                y=z.p.p.left
                if y is not None and y.color==RED:  #sentinal pointer is always black
##                    print "right case 1"
                    z.p.color=BLACK
                    y.color=BLACK
                    z.p.p.color=RED
...
##        self.updateMin()
    ...
    def deleteNode(self,root):
        ...
        if savcolor == BLACK:
            ...
            elif u is not None:  #if u is none then there are no nodes in the tree
                root = u.deleteFixup(root,True)  # x's "parent" is y's old parent
...
##                    print "left case 1"
                    ...
                    w=parent.right
                if (w is not None) and (w.left is None or w.left.color == BLACK) and \
...
##                    print "right case 4"
    ...
    def nextNode(self):
        ...
        if self.right is None and self.p is not None and self.p.left == self:
            node=self.p
        elif self.right is None and self.p is not None and self.p.right == self:
            node=self.p
